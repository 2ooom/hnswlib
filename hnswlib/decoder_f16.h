...
namespace hnswlib {
    ...
    #if defined(USE_SSE)
        if (dim % 4 == 0) return encode_vector_float16_SIMD4;
        else if (dim > 4) return encode_vector_float16_SIMD4Residuals;
    #endif
    #if defined(USE_AVX)
        if (dim % 8 == 0) return encode_vector_float16_SIMD8;
        else if (dim > 8) return encode_vector_float16_SIMD8Residuals;
    #endif
        return encode_vector_float16;
    }

    static
    DECODEFUNC<uint16_t, float> get_fast_float16_decode_func(size_t dim) {
    #if defined(USE_SSE)
        if (dim % 4 == 0) return decode_float16_vector_SIMD4;
        else if (dim > 4) return decode_float16_vector_SIMD4Residuals;
    #endif
    #if defined(USE_AVX)
        if (dim % 8 == 0) return decode_float16_vector_SIMD8;
        else if (dim > 8) return decode_float16_vector_SIMD8Residuals;
    #endif
        return decode_float16_vector;
    }
}